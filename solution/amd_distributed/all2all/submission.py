...
# ---------------- Optimized All2All ROCm impl ----------------
class ROCmOptimizedAllToAll:
    META_DIM = 5  # global_exp, src_rank, src_token, src_k, pad
    ...
    def __init__(self, cfg, rank: int, world_size: int):
        ...
        # ROCm 8卡拓扑优化：0-3 一组，4-7 一组
        self.group_size = 4
        self.group_id = rank // self.group_size
        self.local_rank = rank % self.group_size

        # 创建两个通信组
        self.group0_ranks = list(range(0, 4))
        self.group1_ranks = list(range(4, 8))

        # 所有进程都需要创建相同的组
        self.group0 = dist.new_group(self.group0_ranks)
        self.group1 = dist.new_group(self.group1_ranks)

        self.current_group = self.group0 if self.group_id == 0 else self.group1
        self.current_ranks = self.group0_ranks if self.group_id == 0 else self.group1_ranks
        self.current_group_size = len(self.current_ranks)
    ...
    def dispatch(self, dp_x: torch.Tensor, indices: torch.Tensor):
        ...
        # ---------3. dispatch send_buf to recv_buf --------------
        dist.all_to_all_single(
            recv_buf,
            send_buf,
            output_split_sizes=recv_counts_t.tolist(),
    ...
    def combine(self, out_tokens: torch.Tensor, weights: torch.Tensor,
        ...
        # 2. traverse each token of each local expert of each rank
        for local_eid in range(self.num_local_experts):
            cnt = int(expert_num_tokens[local_eid].item())
            for j in range(cnt):
                # meta info token j of local eid
        ...
        return out_tokens# ---------------- Optimized All2All pytorch impl ----------------
...
def custom_kernel(data: input_t) -> output_t:
    ...
    ata = ROCmOptimizedAllToAll(cfg, rank, world_size)
